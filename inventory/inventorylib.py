...
def update_user_password(username, password):
    ...
    sysvars.dbs.safe.query("update users set password='{}' where name='{}'".format(hashedentry, username))
...
def calc_total_parts_value(inventory):
    pass
...
def createnewinventory(d, output={'message': ''}):
    ...
    existing_table_names = inventory_db.get_table_names()

    try:
        existing_table_names.remove('metadata')
    except:
        # no metadata table for some reason
        pass

    existingitems = [int(tablename) for tablename in existing_table_names]
    if existingitems:
        existingitems.sort()
        output['message'] += 'existing items: {}. '.format(existingitems)
...
def generateandaddorders(**kwargs):
    from iiutilities import dblib, datalib

    settings = {
        'stockdatabase':sysvars.dbs.stock,
        'ordersdatabase':sysvars.dbs.orders,
        'debug':False
    }
    settings.update(**kwargs)
    ...
    stockdatabase = settings['stockdatabase']
    ordersdatabase = settings['ordersdatabase']

    stockgeneratedorders = generateordersfromstock(stockdatabase)['orders']

    if settings['debug']:
        gen_elapsed = (datetime.datetime.now() - start_time).total_seconds()
        print('Generate orders : {}'.format(gen_elapsed))
        start_time = datetime.datetime.now()
    ...
    if settings['debug']:
        remove_elapsed = (datetime.datetime.now() - start_time).total_seconds()
        print('Remove autogen orders : {}'.format(remove_elapsed))
        start_time = datetime.datetime.now()

    makeordermetadata(ordersdatabase)

    if settings['debug']:
        makemeta_elapsed = (datetime.datetime.now() - start_time).total_seconds()
        print('Make meta : {}'.format(makemeta_elapsed))
        start_time = datetime.datetime.now()
    ...
    for supplier, orderitems in stockgeneratedorders.items():
        ...
        newordername = createneworder({'partsdictarray':orderitems}, makeordermetadata=False)['newordername']

        if settings['debug']:
            elapsed = (datetime.datetime.now() - order_start_time).total_seconds()
            print('create new order : {}'.format(elapsed))
        ...
        if settings['debug']:
            elapsed = (datetime.datetime.now() - order_start_time).total_seconds()
            print('recalc part data : {}'.format(elapsed))

    if settings['debug']:
        step_elapsed = (datetime.datetime.now() - start_time).total_seconds()
        print('Make new orders : {}'.format(step_elapsed))
        start_time = datetime.datetime.now()
    ...
    if settings['debug']:
        makemeta_elapsed = (datetime.datetime.now() - start_time).total_seconds()
        print('update orders meta : {}'.format(makemeta_elapsed))
        start_time = datetime.datetime.now()

    ordersdatabase.execute_queue()

    if settings['debug']:
        execute_elapsed = (datetime.datetime.now() - start_time).total_seconds()
        print('Execute queue ({} items) : {}'.format(len(ordersdatabase.queued_queries), execute_elapsed))
        start_time = datetime.datetime.now()
...
def calcstockfromall(**kwargs):
    ...
    This can now be done with a date, for where to stop and calculate the value from. Assuming we have data that goes
    back taht far, that is.
    ...
    # need to add a fixer for date, since they are not getting spaced right 100% of the time
    ...
    if settings['write_stock']:
        ...
        print("not writing values")
        settings['output']['inventory'] = newstockparts
...
def createneworder(d, **kwargs):
    from iiutilities import dblib, datalib

    settings = {
        'output':{'message': ''},
        'makeordermetadata':True
    }
    settings.update(kwargs)

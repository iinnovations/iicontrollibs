...
def generateandaddorders(stockdatabase=sysvars.dirs.dbs.stock, ordersdatabase=sysvars.dirs.dbs.orders):
    ...
    for neworderdatum in neworderdata:
        ...
        dblib.setsinglevalue(ordersdatabase, 'metadata', 'desc', 'autogenerated',
                             "name='" + neworderdatum['name'] + "'")
        dblib.setsinglevalue(ordersdatabase, 'metadata', 'created', datalib.gettimestring(),
                             "name='" + neworderdatum['name'] + "'")
...
def calcstockfromall(**kwargs):
    settings = {
        'inventoriesdatabase': sysvars.dirs.dbs.inventories,
        'stockdatabase': sysvars.dirs.dbs.stock,
        'ordersdatabase':sysvars.dirs.dbs.orders,
    ...
    stock_database = dblib.sqliteDatabase(settings['stockdatabase'])
    inventory_database = dblib.sqliteDatabase(settings['inventoriesdatabase'])
    orders_database = dblib.sqliteDatabase(settings['ordersdatabase'])
    assemblies_database = dblib.sqliteDatabase(settings['assembliesdatabase'])
    ...
    for inventorytablename in inventorytablenames:
        ...
        try:
            ...
            if metaentry['executed']:
                # print('executed')
                inventoryitems = inventory_database.read_table_row(inventorytablename)
...
def makebommetadata(**kwargs):
    ...
    this_database = dblib.sqliteDatabase(settings['database'])
...
def addeditbom(d, output={'message': ''}):
    ...
    if settings['bomdata']['name'] not in bomnames:
        ...
        makebommetadata(database=boms_database.path)
...
def makeordermetadata(database=sysvars.dirs.dbs.orders, **kwargs):
    ...
    the_database = dblib.sqliteDatabase(database)

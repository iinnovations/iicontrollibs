...
def init_netstats_database(path=netstats_dbpath):
    ...
    the_database.create_table('wired',schema)
    the_database.create_table('wireless',schema)
...
def get_and_log_netstats(path=netstats_dbpath, **kwargs):
    ...
    the_database = dblib.sqliteDatabase(path)
    the_database.insert('wired', {'time':gettimestring(), 'download':results.download/1000000,
                                  'upload':results.upload/1000000, 'ping':results.ping})
...
def analyze_and_histo_access_db(dbpath=access_dbpath):
    ...
    access_db_tablenames = access_db.table_names()
...
def table_access_histo_metadata(metadata):
    pass
...
if __name__ == "__main__":
    get_and_log_netstats()
    parse_and_table_nginx_access_log()
    analyze_histo_and_table_access_db()

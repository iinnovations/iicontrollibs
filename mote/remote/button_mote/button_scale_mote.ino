...
// We do this so we can send text to serial when a button event is run
// without bugging out the program
// We still use an interrupt to make sure we don't miss it
byte runDoButton = 0;
...
void setup() {
  ...
  // Scale serial port
  // first double-check mode is set
  iomode[iopins[6]]=0;
  iomode[iopins[5]]=0;
  ...
  int i;
  for (i=0;i<13;i++) {
    // THis is normally set to the same as ioenabled, but for here we are going to disable reporting 
     ioreportenabled[i] = 0;

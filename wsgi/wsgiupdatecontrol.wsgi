def application(environ, start_response):
    ...
    if action == 'setsystemflag':
        ...
        if database and ioid and value:
            query = pilib.makesqliteinsert('ioinfo', [ioid, value], ['id', 'name'])
            try:
                pilib.sqliteinsertsingle(pilib.controldatabase, 'ioinfo', [ioid, value], ['id', 'name'])
            except:
